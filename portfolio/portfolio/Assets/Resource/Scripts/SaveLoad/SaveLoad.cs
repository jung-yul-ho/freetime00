...
public class SaveLoad : MonoBehaviour
{
    ...
    public void Save()
    {
        ...
        for (int i = 0; i < 15; i++)
        {
            ItemNumbers.Add(PlayerInformation.Instance.InvenList[i]);
            if(Inventory.Instance.inventories[i].InitItem != null)
            {
        ...
        int head = PlayerInformation.Instance.HeadItem;
        int body = PlayerInformation.Instance.BodyItem;
        int weapon = PlayerInformation.Instance.WeaponItem;
        int consume = PlayerInformation.Instance.ConsumItem;
        int money = PlayerInformation.Instance.Money;
    ...
    public void Load()
    {
        if(File.Exists(Application.persistentDataPath + "/11.json"))
        {
            ...
            for (int i = 0; i < 15; i++)
            {
                ...
                {
                    PlayerInformation.Instance.InvenList[i] = int.Parse(myjsondata["playeritem"][i].ToString());
                }
            }
            PlayerInformation.Instance.HeadItem = int.Parse(myjsondata["headnumber"].ToString());
            PlayerInformation.Instance.BodyItem = int.Parse(myjsondata["bodynumber"].ToString());
            PlayerInformation.Instance.WeaponItem = int.Parse(myjsondata["weaponnumber"].ToString());
            PlayerInformation.Instance.ConsumItem = int.Parse(myjsondata["consumenumber"].ToString());
        }
        //if (File.Exists(Application.persistentDataPath + "/11.json"))
        //{
        //    int InventoryNum = 0;
